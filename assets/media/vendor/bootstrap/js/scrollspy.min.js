...
class ScrollSpy extends BaseComponent {
  ...
  _maybeEnableSmoothScroll() {
    this._config.smoothScroll &&
      ...
      EventHandler.on(this._config.target, EVENT_CLICK, "[href]", (t) => {
  ...
  _initializeTargetsAndObservables() {
    ...
    const t = SelectorEngine.find("[href]", this._config.target);
    for (const e of t) {
      if (!e.hash || isDisabled(e)) continue;
      const t = SelectorEngine.findOne(e.hash, this._element);
      isVisible(t) &&
  ...
  _activateParents(t) {
    if (t.classList.contains("dropdown-item"))
      ...
      for (const e of SelectorEngine.parents(t, ".nav, .list-group"))
        for (const t of SelectorEngine.prev(e, SELECTOR_LINK_ITEMS))
          t.classList.add("active");
  }
  _clearActiveClass(t) {
    t.classList.remove("active");
    const e = SelectorEngine.find("[href].active", t);
...
if (
  (EventHandler.on(window, EVENT_LOAD_DATA_API, () => {
    for (const t of SelectorEngine.find(SELECTOR_DATA_SPY))
      ScrollSpy.getOrCreateInstance(t);
  }),
  ...
  const t = Joomla.getOptions("bootstrap.scrollspy");
  "object" == typeof t &&
    null !== t &&
    Object.keys(t).forEach((e) => {
      const o = t[e],
        s = {
          offset: o.offset ? o.offset : 10,
          method: o.method ? o.method : "auto",
        };
      o.target && (s.target = o.target);
      const r = Array.from(document.querySelectorAll(e));
      r.length && r.map((t) => new window.bootstrap.Scrollspy(t, s));
    });
}
