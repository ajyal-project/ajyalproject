import {
  E as EventHandler,
  g as getElementFromSelector,
  i as isVisible,
  S as SelectorEngine,
  e as enableDismissTrigger,
  d as defineJQueryPlugin,
  B as BaseComponent,
  l as ScrollBarHelper,
  m as Backdrop,
  F as FocusTrap,
  r as reflow,
  c as isRTL,
} from "./dom.min.js?5.2.3";
const NAME = "modal",
  DATA_KEY = "bs.modal",
  EVENT_KEY = `.${DATA_KEY}`,
...
class Modal extends BaseComponent {
  ...
  show(e) {
    ...
    EventHandler.trigger(this._element, EVENT_SHOW, { relatedTarget: e })
      .defaultPrevented ||
      ((this._isShown = !0),
  ...
  dispose() {
    for (const e of [window, this._dialog]) EventHandler.off(e, EVENT_KEY);
...
EventHandler.on(
  ...
  function (e) {
    ...
    ["A", "AREA"].includes(this.tagName) && e.preventDefault(),
      EventHandler.one(t, EVENT_SHOW, (e) => {
        e.defaultPrevented ||
...
const allowed = { iframe: ["src", "name", "width", "height"] };
if (
  ((Joomla.initialiseModal = (e, t) => {
    e instanceof Element &&
      (new window.bootstrap.Modal(e, t),
      (e.open = () => {
        window.bootstrap.Modal.getInstance(e).show(e);
      }),
      (e.close = () => {
        window.bootstrap.Modal.getInstance(e).hide();
      }),
      e.addEventListener("show.bs.modal", () => {
        if ((Joomla.Modal.setCurrent(e), e.dataset.url)) {
          const t = e.querySelector(".modal-body"),
            i = t.querySelector("iframe");
          if (i) {
            const t = e.querySelector("joomla-field-mediamore");
            t && t.parentNode.removeChild(t), i.parentNode.removeChild(i);
          }
          if (e.dataset.iframe.indexOf("document.getElementById") > 0) {
            const i = e.dataset.iframe.split("+"),
              o = i[1].split('"');
            let s;
            if (
              ((o[0] = o[0].replace(/&quot;/g, '"')),
              document.getElementById(o[1]))
            )
              s = document.getElementById(o[1]).value;
            else {
              s = new Function(`return ${o[0]}`).call(null);
            }
            t.insertAdjacentHTML(
              "afterbegin",
              Joomla.sanitizeHtml(`${i[0]}${s}${i[2]}`, allowed)
            );
          } else
            t.insertAdjacentHTML(
              "afterbegin",
              Joomla.sanitizeHtml(e.dataset.iframe, allowed)
            );
        }
      }),
      e.addEventListener("shown.bs.modal", () => {
        const t = e.querySelector(".modal-body"),
          i = e.querySelector(".modal-header"),
          o = e.querySelector(".modal-footer");
        let s = 0,
          a = 0,
          n = 0,
          l = 0,
        ...
        if (t) {
          if (i) {
            (s = i.getBoundingClientRect().height), (r = t.offsetHeight);
          }
          o &&
            (a = parseFloat(
              getComputedStyle(o, null).height.replace("px", "")
            ));
          const e = parseFloat(
        ...
        if (e.dataset.url) {
          ...
          parseFloat(getComputedStyle(i, null).height.replace("px", "")) > n &&
            ((t.style.maxHeight = n),
            (t.style.overflowY = "auto"),
            (i.style.maxHeight = n - l));
        }
      }),
      e.addEventListener("hide.bs.modal", () => {
        e.querySelector(".modal-body").style.maxHeight = "initial";
      }),
      e.addEventListener("hidden.bs.modal", () => {
        Joomla.Modal.setCurrent("");
      }));
  }),
  (Joomla.iframeButtonClick = (e) => {
    if (!e.iframeSelector || !e.buttonSelector)
      throw new Error("Selector is missing");
    const t = document.querySelector(`${e.iframeSelector} iframe`);
    if (t) {
      const i = t.contentWindow.document.querySelector(e.buttonSelector);
      i && i.click();
    }
  }),
  Joomla && Joomla.getOptions)
) {
  const e = Joomla.getOptions("bootstrap.modal");
  "object" == typeof e &&
    null !== e &&
    Object.keys(e).forEach((t) => {
      const i = e[t],
        o = {
          backdrop: !i.backdrop || i.backdrop,
          keyboard: !i.keyboard || i.keyboard,
          focus: !i.focus || i.focus,
        };
      Array.from(document.querySelectorAll(t)).map((e) =>
        Joomla.initialiseModal(e, o)
      );
    });
}
export { Modal as M };
